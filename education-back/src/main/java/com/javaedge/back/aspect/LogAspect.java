...
public class LogAspect {
    ...
    @Pointcut("execution(* com.javaedge.back.service.*.*(..))")
     ...
     * 对 controller 限流
     */
    @Pointcut("execution(* com.javaedge.back.controller.*.*(..))")
...
//        log.info("IP地址: {}", IpUtils.getIpAddress(request));
    ...
    @Before("execution(* com.javaedge.back.controller.*.getById(..))")
...
//        log.info("Method called: " + joinPoint.getSignature().toShortString());
        ...
        // 获取 entity 类名
        String entityMapperName = "com.javaedge.back.mapper." + controllerClassName.replace("Controller", "Mapper");
        if (articles.contains(controllerClassName)) {
            entityMapperName = "com.javaedge.back.mapper.ArticleMapper";
        } else {
            entityMapperName = "com.javaedge.back.mapper." + controllerClassName.replace("Controller", "Mapper");
        }
